...
function DashboardPage() {
  ...
  // ✅ NEW: theme option (persist)
  // theme: 'off' | 'xmas' | 'rain'
  const [theme, setTheme] = useState(() => {
    try {
      // ưu tiên key mới
      const saved = localStorage.getItem('cronchat_theme')
      if (saved === 'off' || saved === 'xmas' || saved === 'rain') return saved

      // backward compatible với key cũ (cronchat_xmas_theme)
      const legacy = localStorage.getItem('cronchat_xmas_theme')
      if (legacy == null) return 'xmas'          // default ON như trước
      return legacy === '1' ? 'xmas' : 'off'
    } catch {
      return 'xmas'
    }
  })
  ...
  useEffect(() => {
    try {
      localStorage.setItem('cronchat_theme', theme)
    } catch {}
  }, [theme])

  // ✅ handler pass xuống header/sidebar
  ...
  useEffect(() => {
    ...
    document.title = total > 0 ? `📩 (${total}) CronChat` : '💬 CronChat'
  }, [rooms])
      ...
      try {
        ...
        const results = await Promise.all(
          directRooms.map(async (room) => {
            try {
              ...
              const name = r?.full_name || r?.name || room.name || 'Direct chat'
        ...
        setRooms((prev) =>
          (prev || []).map((room) => {
            ...
            const hit = results.find((x) => Number(x.roomId) === Number(room.id))
            return hit && hit.displayName ? { ...room, displayName: hit.displayName } : room
          })
        )
      } catch (err) {
  ...
  const handleSelectRoom = async (roomId) => {
    ...
    try {
      ...
      console.error(err)
      setRooms((prev) =>
        prev.map((r) => (r.id === roomId ? { ...r, unread_count: prevUnread } : r))
      )
    }
  }
  ...
  const handleUploadRoomImages = async ({ roomId, files }) => {
    ...
    for (const f of files) {
      const res = await uploadRoomImage(roomId, f)
      results.push(res)
    }
  ...
  const handleRealtimeMessage = (rawMsg) => {
    ...
    let createdAtRaw = rawMsg.created_at ?? rawMsg.createdAt ?? new Date().toISOString()
  ...
  const handleOpenDirectRoom = (room, targetUser) => {
    const partnerName = targetUser?.full_name || targetUser?.fullName || targetUser?.username
  ...
  return (
    <div className={`dashboard theme-${theme}`}>
      ...
      {theme !== 'off' && (
        <div className={`theme-overlay theme-${theme}`} aria-hidden="true">
          {[...Array(30)].map((_, i) => {
            if (theme === 'xmas') {
              const type =
                i % 5 === 0
                  ...
                  ? 'type-2'
                  : 'type-1'
              return <div key={i} className={`theme-item ${type}`} />
            }

            if (theme === 'rain') {
              const type = i % 6 === 0 ? 'type-2' : 'type-1'
              return <div key={i} className={`theme-item ${type}`} />
            }
